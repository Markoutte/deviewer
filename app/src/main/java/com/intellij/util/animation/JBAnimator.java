 ...
 * @see Animation
 */
public final class JBAnimator implements AutoCloseable {
    ...
    public JBAnimator() {
        this(Thread.SWING_THREAD);
    }

    private JBAnimator(@NotNull Thread threadToUse) {
    ...
    public long animate(@NotNull Collection<@NotNull Animation> animations) {
        ...
        myCurrentAnimatorFuture = myService.schedule(new Runnable() {
            ...
            public void run() {
                ...
                if (wasLate < 0) {
                    // TODO LOG.warn("Negative animation late value");
                    wasLate = 0;
                }
                if (taskId < myRunning.get()) {
                    finalizeRunning();
